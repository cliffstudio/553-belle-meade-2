...
export default function FullWidthSlideshow({
  ...
  useEffect(() => {
    ...
    const syncFlickityLayout = () => {
      if (
        !flickityRef.current ||
        typeof flickityRef.current !== "object" ||
        !("resize" in flickityRef.current)
      ) {
        return;
      }
    ...
    const syncTextHeights = () => {
      ...
      const textBlocks = Array.from(
        carouselRef.current.querySelectorAll<HTMLElement>(".text-wrap"),
      );
    ...
    const syncActiveVideo = (activeIndex: number) => {
      ...
      const videos = Array.from(
        carouselRef.current.querySelectorAll<HTMLVideoElement>(
          "video[data-slide-index]",
        ),
      );
        ...
        if (carouselRef.current) {
          ...
          const animateProgress = (timestamp: number) => {
            const elapsedInCycle =
              (timestamp - cycleStartTime) % totalCycleDuration;
            const activeSegmentIndex =
              Math.floor(elapsedInCycle / SLIDE_DURATION_MS) % slideCount;
            ...
            if (activeSegmentIndex !== activeSlideIndex) {
              ...
              ).select(activeSegmentIndex, false, false);
    ...
    if ("ResizeObserver" in window) {
      ...
      if (sectionRef.current) {
        layoutResizeObserver.observe(sectionRef.current);
      }
      if (carouselRef.current) {
        layoutResizeObserver.observe(carouselRef.current);
      }
    }
    ...
    return () => {
      ...
      if (
        flickityRef.current &&
        typeof flickityRef.current === "object" &&
        "destroy" in flickityRef.current
      ) {
    ...
    <section ref={sectionRef} className="full-width-slideshow-block">
      <div ref={carouselRef} className="carousel flickity-enabled">
        {slides.map((item, index) => {
          ...
          const isVideo =
            mediaType === "video" && (item.video || item.videoUrl);
          ...
          return (
            <div key={item._key || index} className="carousel-cell">
              <div className="media-wrap out-of-opacity">
                ...
                {isVideo && (
                  <div className="fill-space-video-wrap out-of-opacity">
                    <video
                      ...
                      src={
                        item.videoSource === "url" && item.videoUrl
                          ? item.videoUrl
                          : videoUrlFor(item.video)
                      }
                      poster={
                        item.videoPlaceholder
                          ? urlFor(item.videoPlaceholder).url()
                          : undefined
                      }
                      muted
                      playsInline
                ...
                <div
                  className="slideshow-with-border-block__progress"
                  aria-hidden="true"
                >
              ...
              {(item.heading || item.subheading?.length || ctaHref) && (
